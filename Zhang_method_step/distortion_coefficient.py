...
def get_distortion(intrinsic_param, extrinsic_param, pic_coor, real_coor):
    ...
    for i in range(len(pic_coor)):
        for j in range(len(pic_coor[i])):
            # transform as homogeneous coordinate
            single_coor = np.array([(real_coor[i])[j, 0], (real_coor[i])[j, 1], 0, 1])

            # use computed intrinsics and extrinsics to compute points of images
            ...
            # estimate difference between real coordinate and computed coordinate
            d.append(pic_coor[i][j, 0] - u_estim)
            d.append(pic_coor[i][j, 1] - v_estim)
            '''
            # use coordinate of corners to compute
    ...
    k = np.dot(temp, d)
    '''
    #use SVD to compute k in equation D * k = d
