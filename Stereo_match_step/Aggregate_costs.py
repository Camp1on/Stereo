...
def get_path_cost(slice, offset, parameters):
    """
    part of the aggregation Zhang_method_step, finds the minimum costs in a D x M slice (where M = the number of pixels in the
    given direction)
...
def aggregate_costs(cost_volume, parameters, paths):
    """
    second Zhang_method_step of the sgm algorithm, aggregates matching costs for N possible directions (8 in this case).
